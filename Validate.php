...
class Validate
{
    ...
    public function isSldValid(string $sld): bool
    {
        ...
        // Hardcoded SLD length restriction with switch
        $sldLengthRestrictionEnabled = true; // 硬编码开关：true=启用，false=禁用
        
        if ($sldLengthRestrictionEnabled) {
            $minLength = 3; // 硬编码最小长度
            $maxLength = 63; // 硬编码最大长度
            $sldLength = strlen($sld);
            
            if ($sldLength < $minLength) {
